...
const Reading = () => {
  ...
  const handleDelete = (reading_id) => {
    axiosInstance
      .delete(`/reading/delete_reading/${reading_id}`)
      .then((result) => {
        if (result.data.Status) {
          setReading(mreading.filter((item) => item.reading_id !== reading_id));
          setFilterReading(
            filterreading.filter((item) => item.reading_id !== reading_id)
          );
        } else {
          alert(result.data.Error);
        }
  ...
  const handleSearchchange = (em) => {
    ...
    const filterreading = mreading.filter(
      (e) =>
